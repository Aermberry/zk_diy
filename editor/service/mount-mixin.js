...
export const HelperClass = {
  ...
  methods: {
    handleMouseOver(event) {
      ...
      this.$store.commit(
        'viewport/setCurrentHoverInstanceKey',
        this.__crrtInstanceKey__
      )
    ...
    handleClick(event) {
      event.stopPropagation()
      this.$store.commit(
        'viewport/setCurrentEditInstanceKey',
        this.__crrtInstanceKey__
      )
    },
    registerEvent() {
    ...
    __refresh__(instanceKey, instanceDom, setting, instance) {
      ...
      const slotsDom = instanceDom.querySelectorAll('.gaea-slot')
    ...
    __handleDragAdd__(e, parentInstanceKey, gaeaKey, instanceKey, slotName) {
      const _parentInstance = this.$store.state.viewport.instances.get(
        parentInstanceKey
      )
      const componentClass = this.$store.state.application.componentClasses.get(
        gaeaKey
      )
      const crrtInstance = this.$store.state.viewport.instances.get(instanceKey)
  ...
  mounted() {
    ...
    // 注册 当前dom 事件
    this.registerEvent()
    this.__crrtInstanceInfo__ = this.$store.state.viewport.instances.get(
      this.__crrtInstanceKey__
    )
    // 设置当前实例vm对象是自己
    ...
    this.__setting__ = this.$store.state.application.componentSetting.get(
      this.__crrtInstanceInfo__.gaeaKey
    )
